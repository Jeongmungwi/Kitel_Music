...
def get_audio_file_pairs(data_dir):
    ...
    for folder_name in os.listdir(data_dir):
        ...
        if os.path.isdir(folder_path):
            ...
            drums_files = sorted([f for f in os.listdir(folder_path) if f.startswith("drums_") and f.endswith(".wav")])
            for mix_file, drums_file in zip(mixture_files, drums_files):
                mixture_path = os.path.join(folder_path, mix_file)
                drums_path = os.path.join(folder_path, drums_file)
                audio_pairs.append((mixture_path, drums_path))
    return audio_pairs
...
def train_model(data_dir, model, optimizer, criterion, config, num_epochs=15, save_dir='./model_checkpoints', log_file='training_log.txt'):
    ...
    with open(log_file, 'w') as log:
        ...
        for epoch in range(num_epochs):
            ...
            # tqdm을 이용한 진행률 표시
            for mixture_path, drums_path in tqdm(audio_pairs, desc="Training"):
                # 오디오 파일 로드 (이후 device로 이동)
                mixture_wav = load_wav_to_tensor(mixture_path, config['sampling_rate']).to(device)
                drums_wav = load_wav_to_tensor(drums_path, config['sampling_rate']).to(device)
                ...
                drums_mel = mel_spectrogram(drums_wav,
                                            config['n_fft'],
                                            config['num_mels'],
                                            config['sampling_rate'],
                                            config['hop_size'],
                ...
                drums_mel = drums_mel.unsqueeze(0)

                # 모델 예측
                predicted_drums_mel = model(mixture_mel)

                # 손실 계산
                loss = criterion(predicted_drums_mel, drums_mel)
...
def main():
    ...
    # 학습 실행
    train_model(data_dir, model, optimizer, criterion, config, num_epochs=100)
